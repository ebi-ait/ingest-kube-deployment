...
def is_in_dcp(uuid):
    ...
    if requests.get(azul_proj_url):
        logging.info(f'Project {uuid} is published in DCP')
        return True
    return False
    ...
    for publication in publications:
        try:
            if not publication.get('doi'):
                logging.info(f"Project {project['uuid']['uuid']} has no doi. Skipping.")
                continue
...
def patch_project(uuid, project, token=None) -> UpdateStatus:
    try:
        ...
        if not project_patch:
            ...
            return UpdateStatus.UNNECESSARY
        else:
...
if __name__ == "__main__":
    ...
    logging.info('started update-project-catalogue')
    ...
    for uuid in uuids:
        ...
        if project:
            update_status = patch_project(uuid, project, token)
            update_results[update_status.name]+=1
    logging.info(f'update results: {update_results}')
    logging.info('finished update-project-catalogue')
